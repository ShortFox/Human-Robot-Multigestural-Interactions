...
public static class RecorderDefinitions
{
    ...
    {
        return type.IsPrimitive || DefinedTypes.Contains(type) || typeof(IRecordable).IsAssignableFrom(type);
    }
    ...
    public static int NumberOfColumnsNeeded(string typeName)
    {
        ...
        if (typeof(IRecordable).IsAssignableFrom(type))
        {
            ...
            return ((IRecordable)typeInstance).NumberOfColumnsNeeded;
        }

        if (type != null &&
            (type.IsPrimitive || type == typeof(string))) return 1;

        switch (typeName.Split('.').Last())
        {
            case nameof(Vector3):
                return 3;
    ...
    public static string[] GenerateOutput(object value)
    {
        ...
        var x = value switch
        {
            IRecordable recordable => recordable.Record(),
    ...
    public static string[] GenerateHeader(string headerNamePrefix, string typeName)
    {
        ...
        {
            var typeInstance = Activator.CreateInstance(type);
            return ((IRecordable)typeInstance).GetHeader();
        }
