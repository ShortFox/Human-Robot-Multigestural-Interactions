...
public class Recorder : MonoBehaviour
{
    ...
    /// Select the in .
    /// </summary>
    [Tooltip("Time timestamp start point, choose between the start of the recording (Recording Time)," +
             " or since level load (Unity Time).")]
    [SerializeField] private TimeType timeType;

    //[Tooltip("The approximate time interval between each recording frame.")]
    //[SerializeField] private float recordEvery = .02f;
    ...
    private void Awake()
    {
        Time.fixedDeltaTime = (1f / 90f);   // Set fixed delta time to VR headset refresh rate.
    }
    ...
    /// <summary>Initialize the recorder.</summary>
    public void Init()
    {
        ...
        switch (timeType)
        {
            case TimeType.RecordingTime:
                RegisterToRecord(new DelegateRecordable(
                    () => new[] { (Time.time - _recordingStartTime).ToString(CultureInfo.InvariantCulture) },
                    new[] { "time" },
                    1));
                break;
            case TimeType.UnityTime:
                RegisterToRecord(new DelegateRecordable(
                    () => new[] { Time.timeSinceLevelLoad.ToString(CultureInfo.InvariantCulture) },
                    new[] { "UnityTime" },
                    1));
                break;
        }
    ...
    /// <summary>Initialize the recorder with an array of recordable objects.</summary>
    public void Init(IRecordable[] recordables)
    {
        Init();
        foreach(IRecordable r in recordables)
        {
            RegisterToRecord(r);
        }
    }
    ...
    public void StartRecording(string savePath)
    {
        if (_hasRecordingStarted)
        {
            Debug.LogWarning($"{nameof(StartRecording)} was called before the old recording was stopped. Stopping the old recording and resetting...");
